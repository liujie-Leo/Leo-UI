...
-->
<template>
  <div :class="['l-calendar','w-full','border','rounded']">
    ...
    <div class="l-calendar-date py-1 px-2">
      <div class="l-calendar-week flex mb-2">
        <div v-for="item in weeks" :key="item" class="flex-1 text-center text-sm py-3 text-gray-500">{{ item }}</div>
      </div>
      <div class="l-calendar-date grid grid-cols-7 grid-rows-5 gap-y-4 gap-x-1">
        <div
          v-for="item in date"
          :key="item.fullDate"
          :class="['text-center','py-6', 'cursor-pointer','rounded','text-sm',{'border-blue-500 border':isNowDay(item.fullDate)},{'bg-blue-300 active':item.fullDate == active},{'hover:bg-blue-100':item.fullDate != active},{'text-gray-500':!item.isGray},{'text-gray-300':item.isGray}]"
